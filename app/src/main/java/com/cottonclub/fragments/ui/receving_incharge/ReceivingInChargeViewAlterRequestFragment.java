...
public class ReceivingInChargeViewAlterRequestFragment extends Fragment implements View.OnClickListener {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (alterRequestList.size() == 0) {
                    for (DataSnapshot dataSnapshot : snapshot.getChildren()) {
                        ...
                        if (AppSession.getInstance().getSaveLoggedInUser(requireActivity()).equals(Constants.RECEIVING_IN_CHARGE_KM)) {
                            if (alterRequestItem.getBrandName().equals(Constants.KIDS_MAGIC)) {
                                alterRequestList.add(alterRequestItem);
                            }
                        }else if(AppSession.getInstance().getSaveLoggedInUser(requireActivity()).equals(Constants.RECEIVING_IN_CHARGE_BB)){
                            if (alterRequestItem.getBrandName().equals(Constants.BBABY)) {
                                alterRequestList.add(alterRequestItem);
                            }
                        }else if(AppSession.getInstance().getSaveLoggedInUser(requireActivity()).equals(Constants.RECEIVING_IN_CHARGE_CB)){
